...
* Can set up run variables such as number of iterations before method in code (e.g. @Measurement(iterations = 5) ) or on the command line. 
...
* When using the trusty environment on Travis, has to use install : “pip install -r requirements.txt” in the .travis.yml file – couldn’t get anything else to work. (Though other methods worked on precise)
* Can stop travis from building a commit by putting [skip ci] in commit message
* By default the gh-pages branch isn’t built in Travis unless it’s added to the safelist in .travis.ym
...
* You can configure Travis such that it deploys files to github pages after a success
...
## Junit
* In JUnit can test whether an exception has been thrown:
...
* Can also use Expectedexception rule: allows you to verify your code has thrown a specified exception
	@Rule
	public ExpectedException thrown = ExpectedException.none();//returns a rule that expects no exception to be thrown, so won’t affect existing tests

## Original Eclipse January Iterator:
* Using a 3D tensor/dataset as an example,  with the axis labelled {0,1,2} and a shape of {3,3,3} the original iterators in eclipse january iterate through this dataset by incrementing axis 2 first, then axis 1 and finally axis 0. This is iterating in memory order.  i.e.
...
## Summation Results:
* Benchmarked these 3 iterators (original, simple index, and the fast MyIterator), for 4D datasets from 10x10x10x10 to 60x60x60x60. 
...
## Tensor Dot Product:
* Looking at performing the tensorDotProduct, which is the same as numpy.tensordot, would recommend reading that documentation to find out more about what this operation is. 
* I wrote two variations on Eclipse January's tensor dot product, in both the start and end points are labelled (though I've also added comments in other places)
...
## Python scripts:
* Have 3 python scripts in this projects. The .travis.yml file calls update-gh.sh which pulls down a recent copy of the repo. calls the python files and does some data copying and renaming before pushing back to github.
...
* PlotWithSize: contains 1 function plot_size, which plots data with throughput score against axes length (of dataset). There is also a resonable chunk of floating code that calls this function and labels the axes etc
